...
setA <- c("female", "age", "children", "income_factor", "as.factor(wealth)", "employment_agg", "college", "dominant_origin", "(vote_agg >= 1)", "(vote_agg == 0)", "(vote_agg == -.1)") # left_right may be better than vote: we have answers for CN, it is less country-specific, there is less PNR
...
# TODO: choose order of regressors_names: for the moment, Female appears on top, then age, etc.
...
# 
mean(alla$finished[alla$valid == T] != "1") # proportion of dropout iii)
...
mean(!is.na(alla$excluded) & alla$excluded == "QuotaMet") # proportion of quota met i)
sapply(countries, function(c) print(paste(c, round(mean(!is.na(Alla[[c]]$excluded) & Alla[[c]]$excluded == "QuotaMet"), 3))))
mean(!is.na(alla$attention_test) & alla$attention_test != "A little" & (is.na(alla$excluded) | alla$excluded != "QuotaMet"))/mean(is.na(alla$excluded) | alla$excluded != "QuotaMet") # proportion of screened out for inattention ii)
sapply(countries, function(c) print(paste(c, round(mean(!is.na(Alla[[c]]$attention_test) & (Alla[[c]]$attention_test != "A little") & (is.na(Alla[[c]]$excluded) | Alla[[c]]$excluded != "QuotaMet"))/mean(is.na(Alla[[c]]$excluded) | Alla[[c]]$excluded != "QuotaMet"), 3))))
mean(as.numeric(alla$duration) <= 686 & !is.na(alla$attention_test) & alla$attention_test == "A little" & (is.na(alla$excluded) | alla$excluded != "QuotaMet"))/mean(is.na(alla$excluded) | alla$excluded != "QuotaMet") # proportion of screened out for speed iv)
sapply(countries, function(c) print(paste(c, round(mean(as.numeric(Alla[[c]]$duration) <= 686 & !is.na(Alla[[c]]$attention_test) & Alla[[c]]$attention_test == "A little" & (is.na(Alla[[c]]$excluded) | Alla[[c]]$excluded != "QuotaMet"))/mean(is.na(Alla[[c]]$excluded) | Alla[[c]]$excluded != "QuotaMet"), 3))))
