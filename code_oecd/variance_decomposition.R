...
lmg_standard_var_standardized <- barres(data = t(as.matrix(all_policies_var_standardized@lmg)), labels = indexes_lab[c(2:length(indexes_lab))],legend = "% of remaining R-squared", rev = F)
# Note R^2 = 43.34%, metrics are normalized to sum 100%. Analysis adjusted for 20 regressorsthat make up 6.35 pcts. pts of R^2
lmg_standard_var_non_standardized <- barres(data = t(as.matrix(standard_var_non_standardized@lmg)), labels = indexes_lab[c(2:length(indexes_lab))],legend = "% of response variances", rev = F)
