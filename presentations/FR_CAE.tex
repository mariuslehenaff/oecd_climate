...
%	\includegraphics[width=.88\paperwidth]{../figures/FR/Coefplot_SetA_PlotA_index_knowledge.png}
...
\section{Which respondents support climate action?
%policies to fight climate change?
}% Who supportes climate policies % Support for climate policies
...
\begin{frame}{Most people support climate action and about half are willing to change their lifestyle.}
	\begin{figure}[h!]
		\centering		
		\caption{Share of support (somewhat or strongly) for the main national policies.}
		\includegraphics[width=\textwidth]{../figures/country_comparison/opinion_willing_positive_countries.png}
		%\caption{}
		\end{figure}
\end{frame}

\begin{frame}{France support climate policies less than the OECD average.}
	\begin{figure}[h!]
		\centering		
		\caption{Share of support (somewhat or strongly) for the main national policies.}
		\includegraphics[width=\textwidth]{../figures/country_comparison/main_national_policies_positive_countries.png}
		%\caption{}
		\end{figure}
\end{frame}

\begin{frame}{Most policies receive a relative majority support.}
	\begin{figure}[h!]
		\centering		
		\caption{Share of support for the main national policies among non-Indifferent.}
		\includegraphics[width=\textwidth]{../figures/country_comparison/main_national_policies_share_countries.png}
		%\caption{}
		\end{figure}
\end{frame}

\begin{frame}{Global policies are strongly supported.}
	\begin{figure}[h!]
		\centering		
		\caption{Share of support (somewhat or strongly) for the main global policies.}
		\includegraphics[width=\textwidth]{../figures/country_comparison/main_international_policies_positive_countries.png}
		%\caption{}
		\end{figure}
\end{frame}
...
\begin{frame}{Less support among car users, right-wing, or older people.
	%How support varies by socio-demographics
	}

\begin{figure}
	\vspace{-.01cm}
	\includegraphics[width=.84\paperwidth]{../figures/FR/Coefplot_SetAB_index_main_policies.png}
...
\vspace{.1cm}
{\footnotesize $R^2$ = 12.21\%}
\begin{figure}[h!]
\vspace{-.1cm}
\includegraphics[width=.8\textwidth]{../figures/FR/lmg_main_policies_socio_non_standardized} \\
...
% 	\vspace{-0.3cm}
% 	\includegraphics[width=.7\textwidth]{../figures/FR/policies_support_by_Gilets_jaunes_agg.png} % TODO! present results on Gilets_jaunes somewhere: that overall they are more climate friendly, except for the carbon tax with cash transfers
...
\begin{frame}{Beliefs in effectiveness and self-interest are key for policy support.}
	% A. Mechanisms. 
% Regress Outcomes on set C + set A (—> answers the question of what type of thinking needs to changed 
% (We already did nice figures like this, but we needed to reorganize what was a mechanism and what was an outcome) 
...
\begin{figure}
	...
	\includegraphics[height=.95\paperheight]{../figures/FR/Coefplot_SetABC_PlotC_index_main_policies.png}
\end{figure}
\end{frame}

\begin{frame}{The distributional dimension is critical.}
\begin{figure}
	\vspace{-.2cm}
...
\begin{frame}{Carbon tax with cash transfers viewed as regressive
	%Incidence: Carbon tax with cash transfers
...
\begin{frame}{Green infrastructure program viewed as regressive
	%Incidence: Green infrastructure program
	}%\addtocounter{framenumber}{-1}
	\begin{figure}[h!]
	\centering
...
\begin{frame}{Explaining the partisan gap -- Gelbach decomposition\\ Non-left voters support less climate policies, because they...}%\addtocounter{framenumber}{-1}
	% \vspace{-.2cm}
%\addtocounter{framenumber}{-1}
\begin{figure}[h!]
...
%\caption{Could you trust the federal goverment to implement the following policies}
{\tiny \textit{Notes:} Each bar indicates the share of the partisan gap explained by each of the factors for the \textit{index main policies}.\\ The unexplained share is 44\%. Coefficient of left voters indicator variable in partial model is .229, and .104 in full model.}
\end{figure}
\end{frame}

\begin{frame}{Explaining the age gap -- Gelbach decomposition \\ People above 35 support less climate policies, because they...}%\addtocounter{framenumber}{-1}
	%\vspace{-.2cm}	
\begin{figure}[h!]
%\caption{… main climate policies}
\includegraphics[width=.65\textwidth]{../figures/FR/gelbach_young_no_fairness_index_main_policies} \\
%\caption{Could you trust the federal goverment to implement the following policies}
{\tiny \textit{Notes:} Each bar indicates the share of the age gap explained by each of the factors for the \textit{index main policies}.\\ The unexplained share is 17\%. Coefficient of young indicator variable in partial model is .176, and .031 in full model.}
\end{figure}
\end{frame}


\begin{frame}{Explaining the education gap -- Gelbach decomposition}%\addtocounter{framenumber}{-1}
...
%\caption{Could you trust the federal goverment to implement the following policies}
{\tiny \textit{Notes:} Each bar indicates the share of the diploma gap explained by each of the factors for the \textit{index main policies}.\\ The unexplained share is 0\%. Coefficient of college indicator variable in partial model is .128, and .004 in full model.}
\end{figure}
\end{frame}
...
\begin{figure}
	\centering 
	\caption{Treatment effects (of watching informational videos) in regressions of \\support for the main policies on the set (A) of socio-demographics.}
	\includegraphics[width=.7\textwidth]{../figures/FR/main_policies_support_by_treatment.png}
\end{figure}
\end{frame}
...
\begin{figure}
	\centering 
	\caption{Treatment effects (of watching informational videos) in regressions of \\indices of support and willingness to change on the set (A) of socio-demographics.}
	%\includegraphics[width=.7\textwidth]{../figures/FR/policies_support_by_treatment.png}
	%\caption{How important are the factors below in order for you to adopt a sustainable lifestyle (i.e. limit driving, flying, and consumption, cycle more, etc.)?}
	\includegraphics[width=.7\textwidth]{../figures/FR/indices_policies_willing_by_treatment_origin0.png}
\end{figure}
\end{frame}
...
\begin{itemize}
\ip 1. \textcolor{blue}{What do people know and misperceive about CC?} \textit{Knowledge matters for policy support and not all groups have the same knowledge.}
	\begin{itemize}
		\item \textcolor{magenta}{Policy implication:} There is scope to improve knowledge (e.g., importance of reducing emissions and the contribution of human activity to CC) among some groups (e.g., the lower educated, those less exposed to polluting sectors, etc.)
	\end{itemize}

\ip 2. \textcolor{blue}{Which respondents support climate action?} \textit{More support for climate policies among the young, more-educated, left wing, non-car users, and those living in areas where public transport is available.}
	\begin{itemize}
		\item \textcolor{magenta}{Policy implication:} Identify groups to target to increase support for climate policies and make decarbonized alternatives (such as public transport) available.
	\end{itemize}

\ip 3. \textcolor{blue}{Which core ideas make people support or oppose policies?} \textit{Beliefs in the effectiveness of policies, the distributional dimension and whether people think they will win or lose from the policies matter most for policy support.}
	\begin{itemize}
		\item \textcolor{magenta}{Policy implication:} Make sure that the policy package is progressive and that people understand that.
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Key take-home messages for policy-making}
\begin{itemize}
\ip 4. \textcolor{blue}{Can information change attitudes?} \textit{Yes, providing information on climate change and policies can increase support but, the type of information matters.}
	\begin{itemize}
		\item \textcolor{magenta}{Policy implication:} Information on climate impacts may not suffice; need to also convince people on the effectiveness of policies and their fairness.
	\end{itemize}

\ip 5. \textcolor{blue}{Are some policies more popular than others?} \textit{There is majority support for green infrastructure program. There is less/mixed support for ban on combustion-engine cars and carbon tax with cash transfers . Other climate policies (such as ban on polluting vehicles in dense areas or subsidies for low-carbon technologies) are largely supported.}
	\begin{itemize}
		\item \textcolor{magenta}{Policy implication:} Even among popular policies, information can improve support as well as making alternatives available?
	\end{itemize}
...
\begin{frame}{Climate change acknowledged as a serious problem}%\addtocounter{framenumber}{-1}
\begin{figure}[h!]
\centering
\caption{Do you agree or disagree with the following statement: ``Climate change is an important problem."}
\includegraphics[width=.78\paperwidth]{../figures/FR/CC_problem_FR.png}
